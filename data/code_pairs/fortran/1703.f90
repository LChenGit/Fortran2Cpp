...
contains
  ...
  subroutine checkLength(name, value, expected)
    ...
    if (value /= expected) then
      print *, "Length check failed for ", trim(name), ": ", value, " /= ", expected
    else
      print *, "Length check passed for ", trim(name)
    endif
  ...
  subroutine checkString(name, value, expected)
    ...
    if (value /= expected) then
      print *, "String check failed for ", trim(name), ": ", value, " /= ", expected
    else
      print *, "String check passed for ", trim(name)
    endif
  ...
  subroutine checkArray(name, array, expectedSize)
    ...
    if (size(array) /= expectedSize) then
      print *, "Array size check failed for ", trim(name), ": size ", size(array), " /= ", expectedSize
    else
      print *, "Array size check passed for ", trim(name)
    endif
