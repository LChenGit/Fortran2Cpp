...
void testDeque() {
    ...
    dq.pushRight(std::make_shared<IntContainer>(5));
    dq.pushRight(std::make_shared<IntContainer>(1));

    assert(dynamic_cast<IntContainer*>(dq.peekLeft().get())->value == 20);
    std::cout << "Test peekLeft passed" << std::endl;

    assert(dynamic_cast<IntContainer*>(dq.peekRight().get())->value == 1);
    std::cout << "Test peekRight passed" << std::endl;

    assert(dynamic_cast<IntContainer*>(dq.popLeft().get())->value == 20);
    std::cout << "Test popLeft passed" << std::endl;

    assert(dynamic_cast<IntContainer*>(dq.popRight().get())->value == 1);
    std::cout << "Test popRight passed" << std::endl;
}
