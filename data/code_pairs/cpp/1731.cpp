...
const int NF = 55;
float CIA[7][NF]; // This should be initialized similarly to the Fortran version.
float CPR[NF];
...
int main() {
    // Initialize CIA for testing
    for (int i = 0; i < 7; ++i) {
        for (int j = 0; j < NF; ++j) {
            CIA[i][j] = static_cast<float>(i * NF + j + 1); // Simplified initialization
        }
    }
