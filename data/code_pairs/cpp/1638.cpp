...
void SLAMRG(int N1, int N2, const std::vector<float>& A, int STRD1, int STRD2, std::vector<int>& INDEX) {
    ...
    N2SV = N2;
    if (STRD1 > 0) {
        IND1 = 0; // Adjust for 0-based indexing in C++
    } else {
        IND1 = N1 - 1;
    }
    if (STRD2 > 0) {
        IND2 = N1; // Adjust for 0-based indexing in C++
    } else {
        IND2 = N1 + N2 - 1;
    }
    ...
    while (N1SV > 0 && N2SV > 0) {
        if (A[IND1] <= A[IND2]) {
            INDEX[I] = IND1 + 1; // Convert back to 1-based indexing for output consistency
            ++I;
            IND1 += STRD1;
            --N1SV;
        } else {
            INDEX[I] = IND2 + 1; // Convert back to 1-based indexing for output consistency
            ++I;
            IND2 += STRD2;
            --N2SV;
        }
