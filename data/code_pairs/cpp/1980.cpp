...
void check_values(my_c_type_1& my_type, int expected_i, void* expected_nested_c_address,
    ...
    if (my_type.my_nested_type.nested_c_address != expected_nested_c_address) std::abort();
    ...
    if (my_type.c_address != expected_c_address) std::abort();
...
int main() {
    ...
    test_instance.my_nested_type.array[0] = 1;
    test_instance.my_nested_type.array[1] = 2;
    test_instance.my_nested_type.array[2] = 3;
    test_instance.c_address = nullptr;
    test_instance.j = 4;

    // Call the function with expected values
