...
void fporde(const std::vector<double>& x, const std::vector<double>& y, 
            int m, int kx, int ky, const std::vector<double>& tx, int nx, 
            const std::vector<double>& ty, int ny, std::vector<int>& nummer, 
            std::vector<int>& index, int nreg) {
    ...
    for (int im = 0; im < m; im++) {
        ...
        index[num-1] = im + 1; // Adjust for 1-based indexing similar to Fortran
    }
}
