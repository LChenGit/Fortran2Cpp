...
void ppm_gmm_extend_fwd(float input, float& output) {
    // Fallback template function - should not be called if specialized correctly
    static_assert(DIM != DIM, "This function template specialization does not exist.");
}
...
int main() {
    ...
    // Demonstrating the use of template specialization for function overloads
