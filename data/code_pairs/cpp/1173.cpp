...
void unpackpfx(int ng, std::string& call1) {
    if (ng < 60000) {
        ...
        for (int i = 2; i >= 0; --i) { // Indices are 0-based in C++
            int nc = n % 37;
            if (nc >= 0 && nc <= 9) {
                pfx[i] = static_cast<char>(nc + '0');
            } else if (nc >= 10 && nc <= 35) {
                pfx[i] = static_cast<char>(nc - 10 + 'A');
            } else {
                pfx[i] = ' ';
            }
            n = n / 37;
        ...
        int nc = ng - 60000;
        if (nc >= 0 && nc <= 9) {
            call1 = call1.substr(0, i1) + "/" + static_cast<char>(nc + '0');
        } else if (nc >= 10 && nc <= 35) {
            call1 = call1.substr(0, i1) + "/" + static_cast<char>(nc - 10 + 'A');
        } else if (nc >= 36 && nc <= 125) {
            int nc1 = (nc - 26) / 10;
            int nc2 = (nc - 26) % 10;
            call1 = call1.substr(0, i1) + "/" + static_cast<char>(nc1 + '0') + static_cast<char>(nc2 + '0');
        }
    }
}