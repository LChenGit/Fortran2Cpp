...
void vpot(double r, double& v, double& vp, double& vpp) {
    ...
    double v2 = (c6 + (c8 + c10 * x2) * x2) * std::pow(x2, 3);
    double v2p = -(6.0 * c6 + (8.0 * c8 + 10.0 * c10 * x2) * x2) * std::pow(x2, 3) / r;
    double v2pp = (6.0 * 7.0 * c6 + (8.0 * 9.0 * c8 + 10.0 * 11.0 * c10 * x2) * x2) * std::pow(x2, 3) / (r * r);

    if (dy > 1.0) {
        f *= std::exp(-std::min(expmax, std::pow(dy - 1.0, 2)));
