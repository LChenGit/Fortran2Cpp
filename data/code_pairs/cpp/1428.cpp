...
void morse(const std::vector<double>& X, std::vector<double>& V, double& EMORSE, 
           ...
           bool GTEST, bool periodic, const std::vector<double>& boxvec, 
    ...
    for (int J1 = 0; J1 < natoms; ++J1) {
        ...
        for (int J2 = J1 + 1; J2 < natoms; ++J2) {
            ...
            if (periodic) {
                for (int k = 0; k < 3; ++k) {
                    dx[k] -= boxvec[k] * std::round(dx[k] * iboxvec[k]);
                }
            }
...
int main() {
    ...
    bool GTEST = true, periodic = false, use_cutoff = true;

    morse(X, V, EMORSE, natoms, rho, R0, A, GTEST, periodic, boxvec, use_cutoff, rcut);
