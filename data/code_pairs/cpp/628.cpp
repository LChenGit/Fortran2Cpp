...
void ZGEMV(char TRANS, int M, int N, Complex ALPHA, const Complex* A, int LDA,
    ...
    if (TRANS == 'N' || TRANS == 'n') {
        for (int i = 0; i < M; ++i) {
            ...
            for (int j = 0; j < N; ++j) {
                temp += A[i * LDA + j] * X[j * INCX]; // Correct indexing for row-major order in C++
            }
...
int main() {
    ...
    Complex A[] = {Complex(1.0, 0.0), Complex(2.0, 0.0), Complex(3.0, 0.0), Complex(4.0, 0.0)}; // Adjusted to match Fortran's column-major order
